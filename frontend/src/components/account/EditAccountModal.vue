<template>
  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'anthropic_aws'" class="space-y-4">
        <div>
          ...
          <input
            ...
            type="text"
            class="input"
            placeholder="https://api.us-east-1.aws.anthropic.com"
          />
          <p class="input-hint">{{ t('admin.accounts.anthropicAwsBaseUrlHint') }}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <input
            v-model="editAnthropicAwsRegion"
            type="text"
            class="input"
            :placeholder="t('admin.accounts.anthropicAwsRegionPlaceholder')"
          />
      ...
      <div
        v-if="account?.platform === 'anthropic' && (account?.type === 'apikey' || account?.type === 'anthropic_aws')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
      ...
      <div
        v-if="account?.platform === 'anthropic' && (account?.type === 'apikey' || account?.type === 'anthropic_aws') && webSearchGlobalEnabled"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
...
const editAnthropicAwsRegion = ref('')
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'anthropic' && (newAccount.type === 'apikey' || newAccount.type === 'anthropic_aws')) {
  ...
  // Load quota limit for apikey/bedrock/anthropic_aws accounts (bedrock quota is also loaded in its own branch above)
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
  ...
  } else if (newAccount.type === 'anthropic_aws' && newAccount.credentials) {
    const credentials = newAccount.credentials as Record<string, unknown>
    editBaseUrl.value = (credentials.base_url as string) || ''
    editApiKey.value = ''
    editAnthropicAwsWorkspaceId.value = (credentials.workspace_id as string) || ''
    editAnthropicAwsRegion.value = (credentials.aws_region as string) || ''

    // Load pool mode for anthropic_aws
    poolModeEnabled.value = credentials.pool_mode === true
    const retryCount = credentials.pool_mode_retry_count
    poolModeRetryCount.value = (typeof retryCount === 'number' && retryCount >= 0) ? retryCount : DEFAULT_POOL_MODE_RETRY_COUNT

    // Load quota limits for anthropic_aws
    ...
    // Load model restriction for anthropic_aws
    loadModelRestrictionFromMapping(credentials.model_mapping as Record<string, unknown> | undefined)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      updatePayload.credentials = newCredentials
    } else if (props.account.type === 'anthropic_aws') {
      const currentCredentials = (props.account.credentials as Record<string, unknown>) || {}
      const newCredentials: Record<string, unknown> = { ...currentCredentials }
      ...
      // API Key - only update if user provided new value
      if (editApiKey.value.trim()) {
        newCredentials.api_key = editApiKey.value.trim()
      }

      // Workspace ID
      newCredentials.workspace_id = editAnthropicAwsWorkspaceId.value.trim()

      // AWS Region
      newCredentials.aws_region = editAnthropicAwsRegion.value.trim()
    ...
    if (props.account.platform === 'anthropic' && (props.account.type === 'apikey' || props.account.type === 'anthropic_aws')) {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
      const newExtra: Record<string, unknown> = { ...currentExtra }
